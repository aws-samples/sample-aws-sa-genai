...
name = 'analysis_assets_as_code_v1'
...
analysisid = l_analysis['AnalysisId']    
...
if sample_analysis['AnalysisId']  is not None:
    print(name + "'s definition is successfully get!")

#res = func.delete_analysis(qs_session, new_id)
...
"""
new_analysis = func.describe_analysis_definition(qs_session, new_id)
f = open('Assets_as_Code/library/2nd_class_assets/analysis_sheet_library.json')
l_s = json.load(f)
s_name = "Pie"
try:
...
#log = 'log/failures' + '.json'
#file = open(log, 'w')
#file.write(str(faillist))
#file.close()
