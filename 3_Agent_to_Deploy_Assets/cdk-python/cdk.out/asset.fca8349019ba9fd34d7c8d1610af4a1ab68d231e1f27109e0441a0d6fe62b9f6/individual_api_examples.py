...
def start_export_job():
    ...
    response = requests.post(f"{API_BASE_URL}/export", json=payload, headers=get_auth_headers())
...
def check_export_status(export_job_id):
    ...
    response = requests.get(f"{API_BASE_URL}/export/{export_job_id}", json=payload, headers=get_auth_headers())
...
def upload_assets():
    ...
    response = requests.post(f"{API_BASE_URL}/upload", json=payload, headers=get_auth_headers())
...
def import_assets():
    ...
    response = requests.post(f"{API_BASE_URL}/import", json=payload, headers=get_auth_headers())
...
def check_import_status(import_job_id):
    ...
    response = requests.get(f"{API_BASE_URL}/import/{import_job_id}", json=payload, headers=get_auth_headers())
    return response.json()
...
def update_permissions():
    ...
    response = requests.post(f"{API_BASE_URL}/permissions", json=payload, headers=get_auth_headers())
...
if __name__ == "__main__":
    ...
    if export_result:
        ...
        if download_url:
            ...
            response = requests.post(f"{API_BASE_URL}/upload", json=upload_payload, headers=get_auth_headers())
            print(f"Upload result: {response.json()}")
    """
